...
// 折扣因子
  ...
  wifiAC.SetRemoteStationManager("ns3::ConstantRateWifiManager",
   ...
   YansWifiChannelHelper wifiChannelAC;
    wifiChannelAC.SetPropagationDelay("ns3::ConstantSpeedPropagationDelayModel");
    wifiChannelAC.AddPropagationLoss("ns3::FriisPropagationLossModel"); 

    phyAC.SetErrorRateModel("ns3::NistErrorRateModel");
    phyAC.SetChannel(wifiChannelAC.Create());
    ...
    phyAC.Set("TxGain", DoubleValue(1));
    phyAC.Set("RxGain", DoubleValue(1));
  ...
  // std::vector<uint32_t> sources(candidates.begin(), candidates.begin() + std::min(nSources, nNodes - 1));
  //   // 3. 定义目的节点为最后一个节点
  ...
  //       std::cout << "Node " << src << " -> Node " << dst 
  //               << " started at " << st << "s" << std::endl;
  //   }

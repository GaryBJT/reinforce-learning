 ...
 */
#ifndef QoarROUTINGPROTOCOL_H
#define QoarROUTINGPROTOCOL_H

#include "qoar-dpd.h"
#include "qoar-neighbor.h"
#include "qoar-packet.h"
#include "qoar-rqueue.h"
#include "qoar-rtable.h"
...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    ...
    static const uint32_t Qoar_PORT;
    ...
    double CalculateDistance(double srcX,double srcY,double srcZ,Ptr<Ipv4> m_ipv4,Ipv4Address src);
    double GetCitr(Ipv4InterfaceAddress targetInterface);
    ...
    void PlotTrainingResults(std::string filename);
    /**
  ...
  private:
     ...
     */
    void RecvQoar(Ptr<Socket> socket);
    /**
     * Receive RREQ
     * \param p packet
    ...
    Ptr<BasicEnergySource> m_energySource; // 新增能量源指针
    // ns3::Ptr<ns3::BasicEnergySource> m_energySource; // 能量源指针
    double m_alpha;
    double m_gamma;
    double m_a;
    double m_b;
    double m_c;
    Ipv4Address m_dst; // 目的节点的IP地址
    DQN m_qLearning; // DQN instance
    std::string GetNodeAddressString();
    std::string GetAddressString(Ipv4Address addr);
    bool m_enableAdaptiveHello;  // 是否启用自适应Hello间隔
    double m_minHelloInterval;   // 最小Hello间隔(秒)
    double m_maxHelloInterval;   // 最大Hello间隔(秒)
    double m_radioRange;         // 无线电通信范围Rd
    uint32_t m_nodeCount;      ///< 网络中节点的数量
    uint32_t m_maxSpeed;       ///< 节点的最大移动速度(m/s)
};

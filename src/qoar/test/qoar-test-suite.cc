 ...
 */
#include "ns3/qoar-neighbor.h"
#include "ns3/qoar-packet.h"
#include "ns3/qoar-rqueue.h"
#include "ns3/qoar-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("QOAR TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(QoarTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), QoarTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(QoarTYPE_RREP);
 ...
 * \ingroup qoar-test
 *
 * \brief Unit test for QOAR routing table entry
 */
...
struct QoarRqueueTest : public TestCase
{
    QoarRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
QoarRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&QoarRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&QoarRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &QoarRqueueTest::CheckTimeout, this);
 ...
 * \brief Unit test for QOAR routing table entry
 */
struct QoarRtableEntryTest : public TestCase
{
    QoarRtableEntryTest()
 ...
 * \ingroup qoar-test
 *
 * \brief Unit test for QOAR routing table
 */
struct QoarRtableTest : public TestCase
{
    QoarRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup qoar-test
 *
 * \brief QOAR test suite
 */
class QoarTestSuite : public TestSuite
{
  public:
    QoarTestSuite()
        : TestSuite("routing-qoar", UNIT)
    {
        ...
        AddTestCase(new QoarRqueueTest, TestCase::QUICK);
        AddTestCase(new QoarRtableEntryTest, TestCase::QUICK);
        AddTestCase(new QoarRtableTest, TestCase::QUICK);
    }
} g_QoarTestSuite; ///< the test suite

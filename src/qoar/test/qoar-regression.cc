...
#include "qoar-regression.h"

#include "bug-772.h"

#include "ns3/abort.h"
#include "ns3/qoar-helper.h"
 ...
 * \ingroup qoar-test
 *
 * \brief QOAR regression test suite
 */
class QoarRegressionTestSuite : public TestSuite
{
  public:
    QoarRegressionTestSuite()
        : TestSuite("routing-qoar-regression", SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("qoar-chain-regression-test"), TestCase::QUICK);
...
} g_QoarRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("QOAR chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & QOAR
    QoarHelper qoar; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(qoar);
    ...
    streamsUsed += qoar.AssignStreams(*m_nodes, streamsUsed);
    // QOAR uses m_size more streams

...
Ad Hoc On-Demand Distance Vector (QOAR)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand
Distance Vector (QOAR) protocol. The implementation is based on
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 QOAR model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the QOAR-UU implementation by Erik Nordström of
Uppsala University.

Model Description
*****************

The source code for the QOAR model lives in the directory `src/qoar`.

Design
++++++

Class ``ns3::qoar::RoutingProtocol`` implements all functionality of
...
forwarding.  The first one, ``ns3::qoar::RouteOutput``, is used for
locally originated packets, and the second one, ``ns3::qoar::RouteInput``,
...
this functionality are attributes of ``ns3::qoar::RoutingProtocol``.
...
QOAR discovers routes on demand.  Therefore, the QOAR model buffers all
packets while a route request packet (RREQ) is disseminated.
A packet queue is implemented in qoar-rqueue.cc. A smart pointer to
...
* This QOAR implementation can detect the presence of unidirectional
  ...
  This heuristic is taken from QOAR-UU implementation and can be disabled.
...
the assertion from the QOAR RFC that QOAR works over UDP.  This model uses
